...
def core_requirements():
    ...
    reqs_raw = re.search(r"REQUIRES = \[(.*?)\]",
                         Path("setup.py").read_text(), re.S).group(1)
...
def process_requirements(errors, module_requirements, package, reqs):
    ...
    for req in module_requirements:
        if "://" in req:
            errors.append(
                f"{package}[Only pypi dependencies are allowed: {req}]")
        if req.partition("==")[1] == "" and req not in IGNORE_PIN:
            errors.append(
                f"{package}[Please pin requirement {req}, see {URL_PIN}]")
...
def requirements_test_output(reqs):
    ...
    filtered = {
        requirement: modules
        for requirement, modules in reqs.items() if any(
            ...
            has_tests(mdl) for mdl in modules)
    }
