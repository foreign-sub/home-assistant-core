...
def async_process_endpoint(
    ...
    if endpoint_id == 0:  # ZDO
        _async_create_cluster_channel(endpoint,
                                      zha_device,
                                      is_new_join,
    ...
    discovery_infos.extend(
        _async_handle_single_cluster_matches(hass, endpoint, zha_device,
                                             profile_clusters, device_key,
                                             is_new_join))


@callback
def _async_create_cluster_channel(cluster,
                                  zha_device,
                                  is_new_join,
                                  channels=None,
                                  channel_class=None):
    ...
    # incorrectly.
    if hasattr(cluster,
               "ep_attribute") and cluster.ep_attribute == "multistate_input":
...
def async_dispatch_discovery_info(hass, is_new_join, discovery_info):
    ...
    if not discovery_info["channels"]:
        _LOGGER.warning("there are no channels in the discovery info: %s",
                        discovery_info)
        return
    component = discovery_info["component"]
    if is_new_join:
        async_dispatcher_send(hass, ZHA_DISCOVERY_NEW.format(component),
                              discovery_info)
    else:
        hass.data[DATA_ZHA][component][
            discovery_info["unique_id"]] = discovery_info
...
def _async_handle_profile_match(hass, endpoint, profile_clusters, zha_device,
    ...
    in_clusters = [
        endpoint.in_clusters[c] for c in profile_clusters
        if c in endpoint.in_clusters
    ]
    out_clusters = [
        endpoint.out_clusters[c] for c in profile_clusters
    ...
    for cluster in in_clusters:
        _async_create_cluster_channel(cluster,
                                      zha_device,
                                      is_new_join,
                                      channels=channels)
        ...
        for cluster_id in profile_clusters:
            if cluster_id in BINARY_SENSOR_TYPES:
                discovery_info.update({
                    SENSOR_TYPE:
                    BINARY_SENSOR_TYPES.get(cluster_id, UNKNOWN)
                })
                break
...
def _async_handle_single_cluster_matches(hass, endpoint, zha_device,
    ...
    for cluster in endpoint.in_clusters.values():
        ...
        if cluster.cluster_id not in profile_clusters:
            ...
            if cluster.cluster_id == PowerConfiguration.cluster_id and (
                    ...
                    or matched_power_configuration):
                continue
    ...
    for cluster in endpoint.out_clusters.values():
        ...
        if cluster.cluster_id not in profile_clusters:
            ...
            if not (cluster.cluster_id == OnOff.cluster_id
...
def _async_handle_single_cluster_match(hass, zha_device, cluster, device_key,
    ...
    if component == SENSOR:
        discovery_info.update({
            SENSOR_TYPE:
            SENSOR_TYPES.get(cluster.cluster_id, SENSOR_GENERIC)
        })
    if component == BINARY_SENSOR:
        discovery_info.update({
            SENSOR_TYPE:
            BINARY_SENSOR_TYPES.get(cluster.cluster_id, UNKNOWN)
        })
