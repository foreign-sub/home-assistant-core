...
HOST_CONFIG_SCHEMA = vol.Schema({
    ...
    vol.Range(5, 10),
    vol.Optional(CONF_PARALLEL, default=DEFAULT_PARALLEL):
    vol.Range(1, 20),
        ...
        vol.Schema(
            {
                ...
                vol.All(cv.ensure_list, [HOST_CONFIG_SCHEMA]),
...
async def async_setup(hass, config):
    ...
    for host in conf[CONF_HOSTS]:
        ...
        if not conf[CONF_MANUAL]:
            async_track_time_interval(hass, data.update,
    ...
    hass.async_create_task(
        async_load_platform(hass, SENSOR_DOMAIN, DOMAIN,
