...
async def async_setup_entry(hass, config_entry):
    ...
    hass.data[DATA_OPENTHERM_GW][DATA_GATEWAYS][
        config_entry.data[CONF_ID]] = gateway
...
def register_services(hass):
    ...
    service_reset_schema = vol.Schema({
        vol.Required(ATTR_GW_ID):
        vol.All(cv.string, vol.In(hass.data[DATA_OPENTHERM_GW][DATA_GATEWAYS]))
    })
    service_set_clock_schema = vol.Schema({
        vol.Required(ATTR_GW_ID):
        vol.All(cv.string,
                vol.In(hass.data[DATA_OPENTHERM_GW][DATA_GATEWAYS])),
        vol.Optional(ATTR_DATE, default=date.today()):
        cv.date,
    ...
    service_set_oat_schema = vol.Schema({
        ...
        vol.All(cv.string,
                vol.In(hass.data[DATA_OPENTHERM_GW][DATA_GATEWAYS])),
        vol.Required(ATTR_TEMPERATURE):
        vol.All(vol.Coerce(float), vol.Range(min=-40, max=99)),
    })
    ...
    async def set_dhw_ovrd(call):
        ...
        value = await gw_dev.gateway.set_hot_water_ovrd(
            call.data[ATTR_DHW_OVRD])
        gw_dev.status.update({gw_var: value})
        async_dispatcher_send(hass, gw_dev.update_signal, gw_dev.status)
...
async def async_unload_entry(hass, entry):
    ...
    await asyncio.gather(
        hass.config_entries.async_forward_entry_unload(entry,
                                                       COMP_BINARY_SENSOR),
...
class OpenThermGatewayDevice:
    ...
    async def connect_and_subscribe(self):
        ...
        self.status = await self.gateway.connect(self.hass.loop,
                                                 self.device_path)
