...
class ViCareClimate(ClimateDevice):
    ...
    def update(self):
        ...
        try:
            ...
            self._attributes[
                "outside_temperature"] = self._api.getOutsideTemperature()
            ...
            self._attributes[
                "circulationpump_active"] = self._api.getCirculationPumpActive(
                )
            ...
            if self._heating_type == HeatingType.gas:
                ...
                self._attributes[
                    "burner_modulation"] = self._api.getBurnerModulation()
                self._attributes[
                    "boiler_temperature"] = self._api.getBoilerTemperature()

            elif self._heating_type == HeatingType.heatpump:
                self._current_action = self._api.getCompressorActive()

                self._attributes[
                    "return_temperature"] = self._api.getReturnTemperature()
    ...
    def set_hvac_mode(self, hvac_mode):
        ...
        if vicare_mode is None:
            _LOGGER.error("Cannot set invalid vicare mode: %s / %s", hvac_mode,
                          vicare_mode)
            return
