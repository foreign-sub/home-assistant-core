...
class ArestSwitchFunction(ArestSwitchBase):
    ...
    def turn_on(self, **kwargs):
        """Turn the device on."""
        request = requests.get(f"{self._resource}/{self._func}",
                               timeout=10,
                               params={"params": "1"})
    ...
    def turn_off(self, **kwargs):
        ...
        if request.status_code == 200:
            ...
            _LOGGER.error("Can't turn off function %s at %s", self._func,
                          self._resource)
...
class ArestSwitchPin(ArestSwitchBase):
    ...
    def __init__(self, resource, location, name, pin, invert):
        ...
        request = requests.get(f"{self._resource}/mode/{self._pin}/o",
                               timeout=10)
        if request.status_code != 200:
    ...
    def turn_on(self, **kwargs):
        ...
        if request.status_code == 200:
            ...
            _LOGGER.error("Can't turn on pin %s at %s", self._pin,
                          self._resource)
    ...
    def update(self):
        """Get the latest data from aREST API and update the state."""
        try:
            request = requests.get(f"{self._resource}/digital/{self._pin}",
                                   timeout=10)
