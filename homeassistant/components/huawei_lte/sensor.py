...
SENSOR_META = {
    ...
    (KEY_DEVICE_INFORMATION, "SoftwareVersion"):
    dict(name="Software version"),
    ...
    for entid, ent in entreg.entities.items():
        ...
        for sensor in sensors:
            ...
            if ent.unique_id.endswith(f"_{oldsuf}"):
                entreg.async_update_entity(entid,
                                           new_unique_id=sensor.unique_id)
                _LOGGER.debug("Updated entity %s unique id to %s", entid,
                              sensor.unique_id)
