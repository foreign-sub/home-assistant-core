...
class EntityPlatform:
    ...
    @callback
    def _async_schedule_add_entities(self,
                                     new_entities,
    ...
    async def async_add_entities(self, new_entities, update_before_add=False):
        ...
        tasks = [
            self._async_add_entity(entity, update_before_add, entity_registry,
                                   device_registry) for entity in new_entities
        ]
    ...
    async def _async_add_entity(self, entity, update_before_add,
        ...
        elif not hasattr(entity,
                         "async_update") and self.parallel_updates == 0:
            entity.parallel_updates = None
        else:
        ...
        if (entity.entity_id in self.entities
                or entity.entity_id in self.hass.states.async_entity_ids(
                    self.domain)):
    ...
    async def async_reset(self):
        ...
        tasks = [
            self.async_remove_entity(entity_id) for entity_id in self.entities
        ]
