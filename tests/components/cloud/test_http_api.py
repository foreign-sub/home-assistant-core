...
async def test_google_actions_sync(mock_cognito, mock_cloud_login,
                                   cloud_client, aioclient_mock):
...
async def test_google_actions_sync_fails(mock_cognito, mock_cloud_login,
                                         cloud_client, aioclient_mock):
...
async def test_login_view(hass, cloud_client):
    ...
    req = await cloud_client.post("/api/cloud/login",
                                  json={
                                      "email": "my_username",
...
async def test_register_view(mock_cognito, cloud_client):
    """Test logging out."""
    req = await cloud_client.post("/api/cloud/register",
                                  json={
                                      "email": "hello@bla.com",
                                      "password": "falcon42"
                                  })
...
async def test_resend_confirm_view(mock_cognito, cloud_client):
    """Test logging out."""
    req = await cloud_client.post("/api/cloud/resend_confirm",
                                  json={"email": "hello@bla.com"})
    assert req.status == 200
...
async def test_websocket_subscription_reconnect(hass, hass_ws_client,
    ...
    with patch("hass_nabucasa.auth.CognitoAuth.renew_access_token"
               ) as mock_renew, patch(
                   "hass_nabucasa.iot.CloudIoT.connect") as mock_connect:
...
async def test_websocket_update_preferences_no_token(hass, hass_ws_client,
                                                     aioclient_mock, setup_api,
                                                     mock_cloud_login):
...
async def test_enabling_remote_trusted_networks_local4(hass, hass_ws_client,
                                                       setup_api,
                                                       mock_cloud_login):
...
async def test_list_google_entities(hass, hass_ws_client, setup_api,
                                    mock_cloud_login):
    """Test that we can list Google entities."""
    client = await hass_ws_client(hass)
    entity = GoogleEntity(hass, MockConfig(should_expose=lambda *_: False),
...
async def test_update_google_entity(hass, hass_ws_client, setup_api,
                                    mock_cloud_login):
...
async def test_enabling_remote_trusted_proxies_local4(hass, hass_ws_client,
                                                      setup_api,
                                                      mock_cloud_login):
    """Test we cannot enable remote UI when trusted networks active."""
...
async def test_enabling_remote_trusted_proxies_local6(hass, hass_ws_client,
                                                      setup_api,
                                                      mock_cloud_login):
    """Test we cannot enable remote UI when trusted networks active."""
...
async def test_update_alexa_entity(hass, hass_ws_client, setup_api,
                                   mock_cloud_login):
...
async def test_sync_alexa_entities_timeout(hass, hass_ws_client, setup_api,
                                           mock_cloud_login):
...
async def test_sync_alexa_entities_no_token(hass, hass_ws_client, setup_api,
                                            mock_cloud_login):
...
async def test_enable_alexa_state_report_fail(hass, hass_ws_client, setup_api,
                                              mock_cloud_login):
...
async def test_thingtalk_convert_internal(hass, hass_ws_client, setup_api):
    ...
    with patch(
            "homeassistant.components.cloud.http_api.thingtalk.async_convert",
            side_effect=thingtalk.ThingTalkConversionError(
                "Did not understand"),
    ):
        await client.send_json({
            "id": 5,
