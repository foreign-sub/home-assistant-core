...
async def test_form_user(hass):
    ...
    with patch(
            ...
            "pyotgw.pyotgw.disconnect",
            return_value=mock_coro(None)) as mock_pyotgw_disconnect:
...
async def test_form_connection_timeout(hass):
    ...
    with patch("pyotgw.pyotgw.connect",
               side_effect=(asyncio.TimeoutError)) as mock_connect:
        result2 = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {
                CONF_NAME: "Test Entry 1",
                CONF_DEVICE: "socket://192.0.2.254:1234"
            },
        )
...
async def test_form_connection_error(hass):
    ...
    with patch("pyotgw.pyotgw.connect",
               side_effect=(SerialException)) as mock_connect:
        result2 = await hass.config_entries.flow.async_configure(
            result["flow_id"], {
                CONF_NAME: "Test Entry 1",
                CONF_DEVICE: "/dev/ttyUSB0"
...
async def test_options_form(hass):
    ...
    result = await hass.config_entries.options.flow.async_configure(
        ...
        user_input={
            ...
            CONF_PRECISION: PRECISION_HALVES
        },
    )
