...
async def test_manually_configured_platform(hass):
    ...
    assert (await async_setup_component(
        hass, COVER_DOMAIN, {COVER_DOMAIN: {
...
async def test_hmip_cover_shutter(hass, default_mock_hap):
    ...
    await hass.services.async_call("cover",
                                   "stop_cover", {"entity_id": entity_id},
                                   blocking=True)
...
async def test_hmip_cover_slats(hass, default_mock_hap):
    ...
    await hass.services.async_call("cover",
                                   "open_cover_tilt", {"entity_id": entity_id},
                                   blocking=True)
    assert len(hmip_device.mock_calls) == service_call_counter + 1
    ...
    await hass.services.async_call("cover",
                                   "stop_cover_tilt", {"entity_id": entity_id},
                                   blocking=True)
...
async def test_hmip_garage_door_tormatic(hass, default_mock_hap):
    ...
    await async_manipulate_test_data(hass, hmip_device, "doorState",
                                     DoorState.OPEN)
    ha_state = hass.states.get(entity_id)
    assert ha_state.state == STATE_OPEN
    assert ha_state.attributes[ATTR_CURRENT_POSITION] == 100
