...
async def test_manually_configured_platform(hass):
    ...
    assert (await async_setup_component(
        hass, CLIMATE_DOMAIN, {CLIMATE_DOMAIN: {
            "platform": HMIPC_DOMAIN
        }}) is True)
...
async def test_hmip_heating_group_heat(hass, default_mock_hap):
    ...
    await async_manipulate_test_data(hass, hmip_device, "actualTemperature",
                                     22.5)
    ...
    await async_manipulate_test_data(hass, hmip_device, "controlMode",
                                     "MANUAL")
    ha_state = hass.states.get(entity_id)
    ...
    default_mock_hap.home.get_functionalHome(
        IndoorClimateHome).absenceType = AbsenceType.PERMANENT
    await async_manipulate_test_data(hass, hmip_device, "controlMode", "ECO")
    ...
    await async_manipulate_test_data(hass, hmip_device, "floorHeatingMode",
                                     "RADIATOR")
...
async def test_hmip_heating_group_cool(hass, default_mock_hap):
    ...
    await async_manipulate_test_data(hass, hmip_device, "coolingIgnored",
                                     False)
    ha_state = hass.states.get(entity_id)
    ...
    await async_manipulate_test_data(hass, hmip_device, "cooling", True)
    await async_manipulate_test_data(hass, hmip_device, "coolingAllowed",
                                     False)
    await async_manipulate_test_data(hass, hmip_device, "coolingIgnored",
                                     False)
    ha_state = hass.states.get(entity_id)
...
async def test_hmip_climate_services(hass, mock_hap_with_service):
    ...
    assert home.mock_calls[-1][1] == (datetime.datetime(2019, 2, 17, 14,
                                                        0), 18.5)
...
async def test_hmip_heating_group_services(hass, mock_hap_with_service):
    ...
    assert hmip_device.mock_calls[-1][0] == "set_active_profile"
    assert hmip_device.mock_calls[-1][1] == (1, )
    assert (len(hmip_device._connection.mock_calls) == 2  # pylint: disable=protected-access
            )
    ...
    assert hmip_device.mock_calls[-1][0] == "set_active_profile"
    assert hmip_device.mock_calls[-1][1] == (1, )
    assert (len(hmip_device._connection.mock_calls)  # pylint: disable=protected-access
            == 12)
