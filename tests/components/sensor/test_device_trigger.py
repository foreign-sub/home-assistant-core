...
async def test_if_fires_on_state_above(hass, calls):
        ...
        automation.DOMAIN,
        {
            automation.DOMAIN: [{
                ...
                "action": {
                    ...
                    "data_template": {
                        "some":
                        "bat_low {{ trigger.%s }}" % "}} - {{ trigger.".join((
                            "platform",
                            "entity_id",
                            "from_state.state",
                            "to_state.state",
    ...
    assert len(calls) == 1
    assert calls[0].data[
        "some"] == "bat_low device - {} - 9 - 11 - None".format(
            sensor1.entity_id)
...
async def test_if_fires_on_state_below(hass, calls):
    ...
    assert len(calls) == 1
    assert calls[0].data[
        "some"] == "bat_low device - {} - 11 - 9 - None".format(
            sensor1.entity_id)
...
async def test_if_fires_on_state_between(hass, calls):
    ...
    assert len(calls) == 2
    assert calls[1].data[
        "some"] == "bat_low device - {} - 21 - 19 - None".format(
            sensor1.entity_id)
...
async def test_if_fires_on_state_change_with_for(hass, calls):
    ...
    await hass.async_block_till_done()
    assert calls[0].data[
        "some"] == "turn_off device - {} - unknown - 11 - 0:00:05".format(
            sensor1.entity_id)
