...
from homeassistant.components.demo.geo_location import DEFAULT_UNIT_OF_MEASUREMENT
from homeassistant.components.demo.geo_location import DEFAULT_UPDATE_INTERVAL
from homeassistant.components.demo.geo_location import NUMBER_OF_DEMO_DEVICES
...
class TestDemoPlatform(unittest.TestCase):
    ...
    def test_setup_platform(self):
        ...
        with patch("homeassistant.util.dt.utcnow", return_value=utcnow):
            ...
            # generated.
            all_states = [
                self.hass.states.get(entity_id) for entity_id in
                self.hass.states.entity_ids(geo_location.DOMAIN)
            ]
            ...
            for state in all_states:
                ...
                assert (abs(state.attributes["latitude"] -
                            self.hass.config.latitude) < 1.0)
                assert (abs(state.attributes["longitude"] -
